$(function() {
    if ($("#symptomInput").length > 0) {
        $.getAjax("/hf/disease/symptom/list", initCatalog);
    }
...
function getSymptomKnowledge() {
    if (currentSymptom) {
        $.postAjax("/hf/symptom/list/symptom", { symptomKey: currentSymptom }, initKnowledgeTree);
    }
}
...
function initCategoryDetailContent(treedata) {
    ...
    $.postAjax("/hf/symptom/content/symptom", { symptomKey: currentSymptom }, function(data) {
