...
public class PersonCycAssessService extends ServiceSupport<PersonCycAssess> {
	 ...
	 * 加载周期考评自评 首页数据及条件查询
	 */
	public PageResult<PersonnelCycleAssessDTO> searchSelfCycAssessPage(PersonCycAssessQuery perCycAssQuery) {
		Page<PersonnelCycleAssessDTO> page = PageHelper.offsetPage(perCycAssQuery.getOffset(), perCycAssQuery.getLimit());// 分页
	 ...
	 * 医德医风档案管理-->查看-->考评情况
	 */
	public PageResult<PersonnelCycleAssessDTO> viewPersonAssessSituation(PersonCycAssessQuery perCycAssQuery) {
		Page<PersonnelCycleAssessDTO> page = PageHelper.offsetPage(perCycAssQuery.getOffset(), perCycAssQuery.getLimit());// 分页
	...
	/*
	 * 根据操作状态筛选周期考评-科室首页数据及 条件查询
	 */
	public PageResult<PersonnelCycleAssessDTO> searchDepartmentCycAssessPage(PersonCycAssessQuery perCycAssQuery) {
		Page<PersonnelCycleAssessDTO> page = PageHelper.offsetPage(perCycAssQuery.getOffset(), perCycAssQuery.getLimit());// 分页
	...
	/*
	 * 根据操作状态筛选周期考评-机构首页数据及 条件查询
	 */
	public PageResult<PersonnelCycleAssessDTO> organizationScreenPersonAssess(PersonCycAssessQuery perCycAssQuery) {
		Page<PersonnelCycleAssessDTO> page = PageHelper.offsetPage(perCycAssQuery.getOffset(), perCycAssQuery.getLimit());// 分页
		String unitId = perCycAssQuery.getUnitId();
		UnitQuery query = DataPermissionUtil.getUnitQueryDouble(unitId);
		if (query == null) {
	...
	/*
	 * 根据考评周期id查询对应考评信息 来判断该周期内用户是否已有操作记录
	 */
	public PersonnelCycleAssessDTO searchBaseInfo(String assessCycleId, String userId) {
	 ...
	 * @return
	 */
	public PersonnelCycleAssessDTO getCycleAssessExtend(String id) {
	...
	public PageResult<PersonnelCycleAssessDTO> noAssessment(PersonCycAssessQuery personCycAssessQuery, boolean isAgency) {
