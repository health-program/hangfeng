$(function() {
    if ($("#diseaseInput").length > 0) {
        $.getAjax("/hf/disease/list", initCatalog);
    }
...
function getDiseaseSummary() {
    $.postAjax("/hf/disease/summary", { disease: currentDisease }, function(summary) {
...
function showDisease(disease) {
    ...
    if (isLayer()) {
        window.location = '/hf/knowledge/index?diseaseName=' + disease;
    }
    ...
    layer.open({
        ...
        area: [w, h],
        content: '/hf/knowledge/index?diseaseName=' + disease
    });
}
...
function showSymptom(symptom) {
    ...
    if (isLayer()) {
        window.location = '/hf/symptom/index?symptomName=' + symptom;
    }
    ...
    layer.open({
        ...
        maxmin: true, //开启最大化最小化按钮
        area: [w, h],
        content: '/hf/symptom/index?symptomName=' + symptom
    });
}
...
function getDiseaseDiet() {
    $.postAjax("/hf/diet/dietInfo", { diseaseKey: currentDisease }, function(dietList) {
        var html = "";
...
function getDiseaseKnowledge() {
    if (currentDisease) {
        $.postAjax("/hf/knowledge/list/disease", { diseaseKey: currentDisease }, initKnowledgeTree);
    }
}
...
function initCategoryDetailContent(treedata) {
    ...
    $.postAjax("/hf/knowledge/content/disease", { diseaseKey: currentDisease }, function(data) {
